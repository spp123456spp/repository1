...
public class RSAUtil {
	...
	public static String encryptString(Key key, String plaintext) throws Exception {
		...
		try {
			...
			ex.printStackTrace();
			log.error("{} encrypt error: {}", plaintext==null?"":plaintext, ex.getMessage());
			throw ex;
		}
	}
	 ...
	 * @return 原文字符串。
	 */
	public static String decryptString(Key key, String encrypttext) throws Exception {
		...
		try {
			...
			ex.printStackTrace();
			log.error("{} Decryption failed. Cause: {}", encrypttext, ex.getMessage());
			throw ex;
		}
	}
